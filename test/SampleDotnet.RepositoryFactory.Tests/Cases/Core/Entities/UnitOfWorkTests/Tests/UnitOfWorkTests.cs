namespace SampleDotnet.RepositoryFactory.Tests.Cases.Core.Entities.UnitOfWorkTests.Tests;
...
public class UnitOfWorkTests
{
    ...
    public async Task Case_UnitOfWork_Do_Not_Skip_DetectChanges()
    {
        ...
        using (IHost build = host.Build())
        {
            ...
            using (var cancellationTokenSource = new CancellationTokenSource())
            {
                ...
                using (var repository = unitOfWork.CreateRepository<ThirdDbContext>())
                {
                    var user1 = new ThirdDbEntity()
                    {
                ...
                using (var repository = unitOfWork.CreateRepository<ThirdDbContext>())
                {
                    var user1 = await repository.FirstOrDefaultAsync<ThirdDbEntity>(f => f.Surname == "Surname");  // Fetch the entity by surname.
                    ...
                    var user2 = await repository.FirstOrDefaultAsync<ThirdDbEntity>(f => f.Name == "NameUpdated2");  // Fetch the entity by updated name.
    ...
    public async Task Case_UnitOfWork_Rollback()
    {
        ...
        using (IHost build = host.Build())
        {
            ...
            using (var cancellationTokenSource = new CancellationTokenSource())
            {
                ThirdDbEntity createdEntityFromFirstInstance = null;
                ...
                // Second instance: Try to select the non-committed entity.
                {
                    using (IRepository<ThirdDbContext> repo = unitOfWork.CreateRepository<ThirdDbContext>())
                    {
                        ThirdDbEntity? entity = await repo.FirstOrDefaultAsync<ThirdDbEntity>(f => f.Name == "TestName" && f.Surname == "TestSurname", cancellationTokenSource.Token);
                ...
                // Third instance: Try to update the entity's required field to null, tracked by a different DbContext instance.
                {
                    using (IRepository<ThirdDbContext> repo = unitOfWork.CreateRepository<ThirdDbContext>())
                    {
                        ThirdDbEntity? entity = await repo.FirstOrDefaultAsync<ThirdDbEntity>(f => f.Name == "TestName", cancellationTokenSource.Token);
                ...
                // Fourth instance: Ensure the entity state is reverted back after rollback.
                {
                    using (IRepository<ThirdDbContext> repo = unitOfWork.CreateRepository<ThirdDbContext>())
                    {
                        ...
                        entity = await repo.FirstOrDefaultAsync<ThirdDbEntity>(f => f.Name == null, cancellationTokenSource.Token);
                        entity.ShouldBeNull();  // Entity with null name should not exist.

                        var entities = repo.AsQueryable<ThirdDbEntity>().ToList();
    ...
    public async Task Case_UnitOfWork_NestedTransactionsWithSavePoints()
    {
        ...
        using (IHost build = host.Build())
        {
            ...
            using (var cancellationTokenSource = new CancellationTokenSource())
            {
                ...
                using (IRepository<ThirdDbContext> repo = unitOfWork.CreateRepository<ThirdDbContext>())
                {
                    var entity2 = await repo.FirstOrDefaultAsync<ThirdDbEntity>(f => f.Name == "User2");
                ...
                using (IRepository<ThirdDbContext> repo = unitOfWork.CreateRepository<ThirdDbContext>())
                {
                    var entity1 = await repo.FirstOrDefaultAsync<ThirdDbEntity>(f => f.Name == "User1");
                ...
                using (IRepository<ThirdDbContext> repo = unitOfWork.CreateRepository<ThirdDbContext>())
                {
                    var entity3 = await repo.FirstOrDefaultAsync<ThirdDbEntity>(f => f.Surname == "Surname3");
